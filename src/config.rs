...
impl<'a> OutputConfig<'a> {
    ...
    pub fn child(config: DispatchConfig<'a>) -> Self {
        OutputConfig(OutputConfigOptions::Child(config))
    }
    ...
    pub fn file<P>(path: &'a P) -> Self
        where P: ?Sized + AsRef<path::Path>
    {
        OutputConfig(OutputConfigOptions::File {
            path: path.as_ref(),
            line_sep: "\n",
        })
    }
    ...
    /// separator character.
    pub fn file_with_line_sep<P>(path: &'a P, line_sep: &'a str) -> Self
        where P: ?Sized + AsRef<path::Path>
    {
        OutputConfig(OutputConfigOptions::File {
            path: path.as_ref(),
    ...
    pub fn file_with_options<P>(path: &'a P, options: &'a fs::OpenOptions) -> Self
        where P: ?Sized + AsRef<path::Path>
    {
        OutputConfig(OutputConfigOptions::FileOptions {
            path: path.as_ref(),
            options: options,
            line_sep: "\n",
        })
    ...
    /// Log files created using this function will use the specified separator as the newline
    /// separator character.
    pub fn file_with_options_and_line_sep<P>(path: &'a P, options: &'a fs::OpenOptions, line_sep: &'a str) -> Self
        where P: ?Sized + AsRef<path::Path>
    {
        OutputConfig(OutputConfigOptions::FileOptions {
            path: path.as_ref(),
    ...
    pub fn custom(log: Box<api::Logger>) -> OutputConfig<'static> {
        OutputConfig(OutputConfigOptions::Custom(log))
    }
}
...
impl<'a> IntoLog for OutputConfig<'a> {
    fn into_fern_logger(self) -> io::Result<Box<api::Logger>> {
        Ok(match self.0 {
            OutputConfigOptions::Child(config) => config.into_fern_logger()?,
            OutputConfigOptions::File { path, line_sep } => {
                Box::new(loggers::WriterLogger::<fs::File>::with_file(path, line_sep)?)
            }
            OutputConfigOptions::FileOptions { path, options, line_sep } => {
                Box::new(loggers::WriterLogger::<fs::File>::with_file_with_options(path, options, line_sep)?)
            }
            OutputConfigOptions::Stdout => Box::new(loggers::WriterLogger::<io::Stdout>::with_stdout()),
            OutputConfigOptions::Stderr => Box::new(loggers::WriterLogger::<io::Stderr>::with_stderr()),
    ...
    fn into_log(self) -> io::Result<Box<log::Log>> {
        Ok(match self.0 {
            OutputConfigOptions::Child(config) => config.into_log()?,
            OutputConfigOptions::File { path, line_sep } => {
                Box::new(loggers::WriterLogger::<fs::File>::with_file(path, line_sep)?)
            }
            OutputConfigOptions::FileOptions { path, options, line_sep } => {
...
impl<'a> IntoLog for DispatchConfig<'a> {
    fn into_fern_logger(self) -> io::Result<Box<api::Logger>> {
        ...
        let log = loggers::DispatchLogger::new(format, output, level)?;
...
pub fn init_global_logger<L: IntoLog>(config: L, global_log_level: log::LogLevelFilter) -> Result<(), InitError> {
    let log = config.into_log()?;
